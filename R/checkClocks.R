...
checkClocks <- function(x, ...) {
    ...
    sizes <- c( length(checkHorvath), length(checkHannum), length(checkLevine), 
                length(checkSkin), length(checkPedBE), length(checkWu), 
                length(checkTL) )
    n <- c( nrow(coefHorvath[-1, ]), nrow(coefHannum), nrow(coefLevine[-1, ]), 
            nrow(coefSkin[-1, ]), nrow(coefPedBE[-1, ]), nrow(coefWu[-1, ]), 
            nrow(coefTL[-1, ]) )
