...
#'    methylclock:::BMIQcalibration(TestDataset[1:1500, ], goldstandard.beta)
#' }
#'
BMIQcalibration <- function(datM,
    ...
    if (length(goldstandard.beta) > dim(datM)[[2]]) {
        stop("Error in function arguments length(goldstandard.beta)
    ...
    w0.m[intersect(which(beta1.v > th1.v[1]), 
                    which(beta1.v <= th1.v[2])), 2] <- 1
    w0.m[which(beta1.v > th1.v[2]), 3] <- 1

    ### fit type1
    message("Fitting EM beta mixture to goldstandard probes")

    rand.idx <- sample(seq_len(length(beta1.v)), min(c(nfit, length(beta1.v))),
                                                            replace = FALSE )
    em1.o <- blc(matrix(beta1.v[rand.idx], ncol = 1),
                    w = w0.m[rand.idx, ],
    ...
    if (plots) {
        message("Check")
        tmpL.v <- as.vector(rmultinom(seq_len(nL), length(beta1.v), 
                                        prob = em1.o$eta))
        tmpB.v <- vector()
        for (l in seq_len(nL)) {
        ...
        legend( x = 0.5, y = 3, legend = c("obs", "fit"),
                fill = c("black", "green"), bty = "n" )
    }
    ...
    for (ii in seq_len(dim(datM)[[1]])) {
        ...
        w0.m[intersect(which(beta2.v > th2.v[1]), 
                        which(beta2.v <= th2.v[2])), 2] <- 1
        ...
        rand.idx <- sample(seq_len(length(beta2.v)), 
                            min(c(nfit, length(beta2.v)),
                            na.rm = TRUE), replace = FALSE)
        em2.o <- blc2(
            Y = matrix(beta2.v[rand.idx], ncol = 1),
            w = w0.m[rand.idx, ], maxiter = niter,
            tol = tol, verbose = TRUE )
    
        message("Done")
    
        # for type II probes assign to state 
        # unmethylated, hemi or full methylation
        ...
        ### generate plot
        if (plots) {
            tmpL.v <- as.vector(rmultinom(seq_len(nL), length(beta2.v), 
                                            prob = em2.o$eta))
            tmpB.v <- vector()

            for (lt in seq_len(nL)) {
                tmpB.v <- c(tmpB.v, rbeta(tmpL.v[lt], 
                                            em2.o$a[lt, 1], em2.o$b[lt, 1]))
            }
            plot(density(beta2.v), main = paste("Type2fit-", 
                                            sampleID, sep = ""))
            d.o <- density(tmpB.v)
            points(d.o$x, d.o$y, col = "green", type = "l")
            legend(
                x = 0.5, y = 3, legend = c("obs", "fit"),
                fill = c("black", "green"), bty = "n" )
        }
