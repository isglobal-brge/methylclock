...
- **Horvath's skin+blood clock (skinHorvath)**: Epigenetic clock for skin and blood cells. It uses 391 CpGs described in @horvath2018epigenetic. It was trained using 450K and EPIC arrays in skin and blood sampels.
...
- **"guintivano dlpfc"**: Reference generated from @guintivano2013cell. It includes dorsolateral prefrontal cortex, NeuN_neg and NeuN_pos.
...
- **"placenta"**: References from @planet_cells, @planet_pkg. It includes placental cells from third trimester Trophoblasts, Stromal, Hofbauer, Endothelial, nRBC, Syncytiotrophoblast.
...
By default all available clocks (Horvath, Hannum, Levine, BNN, skinHorvath, ...) are estimated. One may select a set of clocks by using the argument `clocks` as follows:
...
```{r get_gse58045, echo=FALSE, eval=FALSE}
...
```{r get_gse19711, echo=FALSE, eval=FALSE}
# ff <- "c:/juan/CREAL/BayesianPrediction/Bayesian_clock/paper"
