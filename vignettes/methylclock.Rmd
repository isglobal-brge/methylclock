...
This manual describes how to estimate chronological and gestational DNA methylation (DNAm) age as well as other age-related biomarkers using different methylation clocks. The package includes the following estimators:
...
- **Levine's clock** (also know as DNAm PhenoAge): It uses 513 CpGs described in @levine2018epigenetic. It was trained using 27K, 450K and EPIC arrays in blood samples.
...
For chronological DNAm age (in months):
...
The main function to estimate chronological DNAm age is called `DNAmAge` while the gestational DNAm age is estimated using `DNAmGA` function. Both functions have similar input arguments. Next subsections detail some of the important issues to be consider before computind DNAm clocks. 
...
# Chronological DNAm age estimation
